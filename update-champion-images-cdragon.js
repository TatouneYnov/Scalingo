...
// CDN Community Dragon - permet CORS et est très fiable
...
async function updateChampionImages() {
  try {
    console.log('🎨 Mise à jour des images via Community Dragon CDN...\n');
    ...
    // Mapping des IDs de champions (certains champions ont des IDs numériques dans CommunityDragon)
    ...
    for (const champion of champions) {
      const champId = championIdMapping[champion.id];
      
      if (!champId) {
        console.log(`⚠ ${champion.name} (${champion.id}) - ID non trouvé`);
        continue;
      }
      
      const imageUrl = `${CDN_URL}/${champId}.png`;
      ...
      try {
        ...
        updatedCount++;
        console.log(`✓ ${champion.name} -> ${champId}.png`);
      } catch (err) {
